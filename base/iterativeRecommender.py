...
class IterativeRecommender(Recommender):
    ...
    def ranking_performance(self,epoch):
        ...
        top = self.ranking['-topN'].split(',') #return a list
        top = [int(num) for num in top] #turn the elements into int
        N = max(top) #only focus on the max num
        recList = {}
        print('Evaluating...')
        for user in self.data.testSet_u:
            candidates = self.predictForRanking(user) #MHCN
