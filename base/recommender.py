...
class Recommender(object):
    ...
    def execute(self):
        #MRO: MHCN->social->graph->deep->iterative->recommender
        self.readConfiguration() #MHCN-social-deep-iterative-recommender
        self.initializing_log() #recommender
        if self.foldInfo == '[1]':
            self.printAlgorConfig() #social-iterative-recommender
        ...
        if self.isLoadModel:
            ...
            self.initModel() #MHCN-deep-iterative
            ...
            try:
                if self.evalSettings.contains('-tf'):
                    ...
                    self.trainModel() #MHCN
            except ImportError:
                self.trainModel()
        #rating prediction or item ranking
        print('Predicting %s...' %self.foldInfo)
        if self.ranking.isMainOn():
            self.evalRanking() #recommender
        else:
            self.evalRatings()
        #save model
        if self.isSaveModel:
